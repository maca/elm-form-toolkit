module Internal.Field exposing
    ...
    , Msg(..), update, updateValue
    , init, isBlank, map
    , updateAttributes
    , identifier, inputType, max, min, name, value
    , label, hint, placeholder, options
    , isGroup, isRequired, isAutocompleteable, selectionStart
    , errors, setErrors
    , inputIdString
    , inputStringToValue
    )
...
@docs isGroup, isRequired, isAutocompleteable, selectionStart
...
-}
type Msg id
    = InputChanged (List Int) Internal.Value.Value Int
...
update msg input =
    case msg of
        InputChanged path val cursorPos ->
            updateAt path
                (Tree.updateValue
                    (\attrs ->
                        { attrs
...
selectionStart : Field id err -> Int
selectionStart input =
    Tree.value input |> .selectionStart
